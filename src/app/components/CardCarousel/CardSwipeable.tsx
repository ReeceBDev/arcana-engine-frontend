...
const CardSwipeable = forwardRef<CarouselDraggableSnapHandle, CardSwipeableProps>(function CardCarouselSwipeable({ onIndexChange, startingIndex }, ref) {
    ...
    const animateSwitch = useCallback((targetIndex: number) => {
        ...
        gsap.to(card, {
            ...
            ease: 'power2.in',
            onComplete: () => {
                console.debug(`intendedIndexRef setting to ${intendedIndexRef.current} (onComplete)`);
    ...
    const next = useCallback(() => {
        console.debug(`Next pressed (next) - intendedIndex: ${intendedIndexRef.current} → ${intendedIndexRef.current + 1}`);
        intendedIndexRef.current = intendedIndexRef.current + 1;
        console.debug(`intendedIndexRef set to ${intendedIndexRef.current} (next)`);
        animateSwitch(intendedIndexRef.current);
    }, [animateSwitch]);

    const previous = useCallback(() => {
        console.debug(`Previous pressed (previous) - intendedIndex: ${intendedIndexRef.current} → ${intendedIndexRef.current - 1}`);
        intendedIndexRef.current = intendedIndexRef.current - 1;
        console.debug(`intendedIndexRef set to ${intendedIndexRef.current} (previous)`);
        animateSwitch(intendedIndexRef.current);
    }, [animateSwitch]);
